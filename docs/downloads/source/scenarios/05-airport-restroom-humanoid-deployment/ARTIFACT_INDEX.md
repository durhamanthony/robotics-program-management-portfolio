# Artifact Index — Airport Restroom Humanoid Pilot

These are completed fictional scenario documents, not empty templates.

| Artifact | Perspective / purpose | Status |
|---|---|---|
| CASE_STUDY.md | Integrated operating story and three viewpoints | Complete |
| RESEARCH_AND_ASSUMPTIONS.md | Public sources, fictional inputs, claim rules | Complete |
| INTEGRATED_PROGRAM_CHARTER.md | Authority, objectives, gates, exclusions | Complete |
| STATEMENT_OF_WORK.md | Seller/integrator contract deliverables | Complete |
| BUSINESS_CASE_AND_TCO.md | Pilot and expansion decisions | Complete |
| PHASE_2_RAAS_EXPANSION_BUSINESS_CASE.md | Scaled customer economics and decision gates | Complete |
| PHASE_2_24_HOUR_OPERATING_PLAN.md | Staggered continuous-terminal service model | Complete |
| RAAS_PRICING_AND_UNIT_ECONOMICS.md | Customer ceiling and provider sustainability screen | Complete |
| 24_HOUR_HUMANOID_OPPORTUNITY_ROADMAP.md | Ranked adjacent 24-hour workflows | Complete |
| BASIS_OF_ESTIMATE_AND_SENSITIVITY.md | Arithmetic, uncertainty, quote requirements | Complete |
| INTEGRATED_MASTER_SCHEDULE.csv | Cross-party plan and gates | Complete |
| BUDGET.csv | Authorization, commitment, actual, variance | Complete |
| RACI.csv | Responsible, Accountable, Consulted, and Informed assignments | Complete |
| RAID.csv | Risk, assumption, issue, and dependency register | Complete |
| REQUIREMENTS_TRACEABILITY.csv | Requirement-to-result evidence | Complete |
| SAFETY_QUALITY_AND_ACCEPTANCE_PLAN.md | Hazard controls, quality audit, acceptance | Complete |
| SELLER_INTEGRATOR_PROJECT_PLAN.md | Seller Project Manager work package | Complete |
| MANUFACTURER_PROJECT_PLAN.md | Manufacturer Project Manager work package | Complete |
| AIRPORT_OWNER_IMPLEMENTATION_PLAN.md | Buyer/operator Project Manager work package | Complete |
| DAILY_OPERATIONS_AND_EXCEPTION_RUNBOOK.md | Daily release, mission, stop, escalation | Complete |
| CYBERSECURITY_PRIVACY_AND_DATA_PLAN.md | Data, access, privacy, remote support | Complete |
| TRAINING_CHANGE_AND_WORKFORCE_PLAN.md | Roles, assessments, workforce participation | Complete |
| BENEFITS_REALIZATION_PLAN.md | Benefits ownership and anti-double-counting rules | Complete |
| WEEKLY_STATUS_REPORT.md | Final integrated status | Complete |
| CLOSEOUT_AND_HANDOFF.md | Final acceptance, handoff, cost, lessons | Complete |
